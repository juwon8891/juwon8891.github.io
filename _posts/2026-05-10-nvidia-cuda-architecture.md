---
layout: post
title: "NVIDIA CUDA 컴파일 과정 및 GPU 아키텍처 심화"
date: 2026-05-10
categories: [CUDA, NVIDIA, GPU, PTX, SASS, NCCL, Ray, Deep-Learning]
---
...
**PTX의 역할**:
1. **포워드 호환성**:
   - 새로운 GPU에서 **JIT(Just-In-Time) 컴파일**로 SASS 생성
   - 예: SM 8.0 PTX → SM 9.0 GPU에서 자동 컴파일
2. **최적화 기회**:
   - 드라이버가 런타임에 GPU별 최적화 적용
3. **인라인 어셈블리**:
...
**SASS의 특징**:
1. **아키텍처 종속성**:
   - SM 8.0 SASS ≠ SM 9.0 SASS (호환 불가)
2. **최대 성능**:
   - GPU의 모든 하드웨어 기능 활용 (Tensor Core, RT Core 등)
3. **디버깅 어려움**:
...
**NCCL의 역할**:
1. **GPU 간 통신**: NVLink, PCIe, InfiniBand 자동 선택
2. **Topology 최적화**: GPU 물리 배치에 따른 최적 경로 계산
3. **중복 제거**: 네트워크 대역폭 최대화
