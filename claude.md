...
**제목 작성 규칙**:
- ✅ **문서 내 H1 제목**: "Image Build", "Scaling", "AuthN & AuthZ" (핵심만, Week 번호 없음)
- ✅ **네비게이션 레이블** (mkdocs.yml): "Week 1 - Image Build", "Week 3 - Scaling" (Week 번호 포함)
  - 장점: 문서는 간결, 네비게이션은 순서 파악 용이
- ❌ **과장 금지**: "완전 정복", "완성하는", "엔터프라이즈급" 같은 마케팅 문구 제거
- ❌ **중복 제거**: 섹션 제목에서 카테고리명 반복 금지 (예: EKS 섹션 내 "EKS 소개" → "소개")
- ✅ **H1 필수**: 모든 문서는 H1 제목으로 시작 (MkDocs 네비게이션 일관성)
...
**약어/풀네임 규칙**:
- ✅ **첫 등장 시에만** 풀네임 표기: `MIG(Multi-Instance GPU)` → 이후에는 `MIG`만 사용
- ❌ **반복 금지**: 같은 문서 내에서 동일 약어에 괄호 풀네임을 두 번 이상 쓰지 않음

**톤앤매너 규칙**:
- ✅ **~다/이다체 사용**: 기술 블로그 표준 스타일 (`정리한다`, `사용한다`, `이다`, `된다`)
- ❌ **합니다/입니다체 금지**: `정리합니다`, `사용합니다`, `입니다` 형태 사용하지 않음
- ✅ **H1 바로 아래 한 줄 요약 blockquote**: `> 핵심 내용을 한 줄로 정리한다.` 형태로 추가
- ❌ **구어체/반말 금지**: `~해보자`, `~알아보자`, `~보도록 하자` 같은 독자 유도 표현 사용하지 않음
- ❌ **이모지 헤더 금지**: `## 🚀 제목` 형태 사용하지 않음
- ✅ **참고 링크**: 문서 맨 아래 `## 참고` 섹션에 모아서 정리

### 가독성 개선 규칙

**표 vs Bullet List 사용 기준**:
- ✅ **표 사용**: 비교할 때만 (A vs B, Before vs After, 항목별 여러 속성 비교)
  ```markdown
  | 구분 | IPMI | Redfish |
  |------|------|---------|
  | 프로토콜 | UDP | HTTP/HTTPS |
  | 데이터 형식 | 바이너리 | JSON |
  ```
- ✅ **Bullet List 사용**: 단순 나열, 특징, 해결책, 장점 등
  ```markdown
  **IPMI의 한계**:
  - 바이너리 프로토콜: 사람이 읽기 어려움
  - UDP 기반: 상태 비저장, 신뢰성 낮음
  - 약한 보안: MD5/SHA-1 인증
  ```
- ❌ **표를 Bullet List로 잘못 사용한 예**:
  ```markdown
  | Redfish 특징 | 장점 |
  |-------------|------|
  | JSON 기반 | 가독성 높음 |
  ```
  위 경우는 비교가 아니라 단순 나열이므로 Bullet List가 적합

**Bullet List를 표로 변환 (4개 이상 + 레이블:설명 형식)**:
- ❌ **가독성 낮음**: 
  ```markdown
  - **레이블 1**: 긴 설명 텍스트
  - **레이블 2**: 긴 설명 텍스트
  - **레이블 3**: 긴 설명 텍스트
  - **레이블 4**: 긴 설명 텍스트 (4개 이상)
  ```
- ✅ **가독성 높음**: 
  ```markdown
  | 레이블 | 설명 |
...
- 각 항목이 "**레이블**: 설명" 형식
- 레이블이 짧고 설명이 긴 경우
- 단, 비교가 아닌 단순 나열은 Bullet List 우선

**Mermaid 다이어그램 색상**:
- ❌ **색상 강제 지정 금지**: `style A fill:#f96,stroke:#333` 같은 색상 코드 사용 금지
- ✅ **테마 자동 적용**: MkDocs Material 테마가 다크모드/라이트모드에 맞춰 자동으로 색상 조정
- ❌ **다크모드 가독성**: 밝은 색상 강제 지정 시 다크모드에서 눈 아픔
- ✅ **구조만 표현**: 색상 없이 노드와 화살표로 구조만 표현
...
**리스트 가독성 (번호 매긴 리스트 & Bullet List)**:
- ❌ **한 줄로 쭉 쓰기**: 항목을 한 줄로 붙여 쓰면 가독성 떨어짐
- ✅ **문단 나누기**: 각 항목 사이에 빈 줄 추가하여 명확히 구분
- ⚠️ **적용 시점**: 항목이 3개 이상이고 각 항목이 긴 경우 (짧은 단어 나열은 그대로 OK)
