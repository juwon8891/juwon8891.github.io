...
> 컨테이너 이미지 빌드, Kustomize, GitOps 기본 개념을 학습한다.
...
| 문제 영역 | 핵심 문제 |
|----------|----------|
| **인적 오류** | SSH 수동 배포, 체크리스트 의존, 단계 누락 위험 |
| **환경 불일치** | Dev/Staging/Prod 간 미묘한 차이, "내 컴퓨터에서는 됨" 문제 |
| **롤백 복잡성** | 이전 버전 복구 어려움, 배포 이력 추적 곤란 |
| **변경 추적** | 누가/언제/무엇을/왜 변경했는지 불명확, 감사 로그 부재 |
| **멀티 클러스터** | 여러 클러스터 간 설정 동기화 어려움, Configuration Drift 감지 불가 |

**GitOps의 혁명적 접근:**

GitOps는 Git 저장소를 **단일 소스(Single Source of Truth)**로 사용하여 인프라를 코드로 제공하는 방법론 및 관행이다.
...
- 원하는 상태(Desired State)를 명시적으로 선언
- Git의 모든 기능(브랜치, 태그, 커밋 히스토리) 활용
- Git 변경사항이 자동으로 클러스터에 반영
- 모든 변경사항이 Git 로그에 기록됨
...
여러 곳에 분산된 정보는 불일치를 야기한다. GitOps는 **오직 Git 저장소**만을 신뢰할 수 있는 정보의 원천으로 간주한다.
...
- 전체 이력을 로컬에도 보관
- Feature 브랜치로 안전한 실험 가능
- Pull Request로 변경사항 검토
- 이전 커밋으로 쉽게 되돌리기
- 이슈, 위키, 프로젝트 보드 등 풍부한 기능

#### 2.2 모든 것은 코드로 표현 (Treat everything as code)

**Infrastructure as Code (IaC)를 넘어서:**

GitOps는 단순히 인프라만이 아니라 **모든 것**을 코드로 관리한다:
...
모든 운영 작업은 Git Pull Request를 통해 수행된다. 이는 다음을 의미한다:
...
- 모든 변경사항을 팀원이 검토
- CI 파이프라인에서 YAML 유효성, 보안 스캔 등 수행
- CODEOWNERS 파일로 필수 승인자 지정
- 누가, 언제, 왜 변경했는지 Git 로그에 기록
- 이전 커밋으로 쉽게 되돌리기
...
[OpenGitOps](https://opengitops.dev/)는 CNCF(Cloud Native Computing Foundation) 산하의 워킹 그룹으로, GitOps의 공식 표준을 정의하고 있다.

**OpenGitOps의 목표:**

- 벤더 중립적인 GitOps 원칙 정의
- 다양한 도구 간 호환성 보장
- 업계 검증된 패턴 공유
- GitOps 생태계 발전
...
- 언제든 이전 시점으로 돌아갈 수 있음
- 두 버전 간의 차이를 명확히 확인 (git diff)
- 환경별로 다른 버전 유지 가능
...
- 클러스터 내부에 ArgoCD나 Flux 설치
- 에이전트가 모니터링할 Git 저장소 지정
- 설정된 간격(예: 3분)마다 Git 저장소 확인
- 새로운 커밋이 있으면 자동으로 감지
- 변경사항을 클러스터에 적용
...
- CI 파이프라인이 클러스터 접근 권한 불필요
- 클러스터가 외부로 나가는 연결만 필요
- 누군가 수동으로 변경해도 자동으로 Git 상태로 복구
- 각 클러스터가 독립적으로 Git에서 pull
...
- 시스템이 스스로 문제를 복구
- 구성 표류(Configuration Drift) 자동 감지 및 수정
- 항상 Git에 정의된 상태 유지
- 모든 조정 작업이 로그로 기록
...
GitOps는 개발자들이 이미 익숙한 Git 워크플로를 그대로 활용한다. 새로운 도구나 프로세스를 학습할 필요가 없다.
...
모든 변경은 Pull Request를 통해 리뷰되므로, 문제를 배포 전에 발견할 수 있다.
...
GitOps 도구는 실제 클러스터 상태와 Git 상태의 차이를 지속적으로 감지한다.
...
- CI가 클러스터 접근 권한 불필요
- 모든 변경이 Git 로그에 기록
- SOC2, HIPAA 등 컴플라이언스 요구사항 충족

#### 4.3 가시성 및 감사 (Visibility and Audit)

**완벽한 변경 이력:**

Git는 모든 변경사항을 타임스탬프, 작성자, 이유와 함께 기록한다.
...
GitOps 도구들은 풍부한 UI를 제공하여 배포 상태를 한눈에 파악할 수 있다.
...
GitOps를 사용하면 여러 쿠버네티스 클러스터를 일관되게 관리할 수 있다.
...
- 모든 클러스터가 같은 Git 저장소 사용
- 변경사항이 모든 클러스터에 자동 반영
- 클러스터 간 차이 즉시 발견
- 모든 클러스터를 한 번에 이전 버전으로 복구
...
CI는 코드 변경사항을 검증하고 컨테이너 이미지를 빌드하는 역할을 담당한다.
...
CD는 Git 저장소의 매니페스트를 쿠버네티스 클러스터에 배포하는 역할을 담당한다.
...
GitOps 엔진(Argo CD, Flux 등)은 CD 파이프라인의 핵심으로, **4가지 주요 작업**으로 구성된 GitOps 루프를 실행한다.
...
Git 저장소에 정의된 매니페스트를 쿠버네티스 클러스터에 배포한다.
...
Git에 정의된 원하는 상태와 클러스터의 실제 상태를 비교하여 차이를 감지한다.
...
감지된 차이를 해결하기 위해 Git 상태로 클러스터를 복구한다.
...
쿠버네티스에서 GitOps를 구현하려면 **최소 2개의 Git 저장소**가 필요한다.
...
컨테이너 레지스트리는 Docker 이미지를 저장하고 배포하는 중앙 저장소이다. Git이 소스 코드를 관리하듯이, 컨테이너 레지스트리는 빌드된 이미지를 관리한다.
...
보안을 위해 비밀번호 대신 Access Token 사용을 권장한다.
...
Quay.io의 강력한 기능인 Robot 계정을 사용하면 CI/CD 파이프라인에서 안전하게 인증할 수 있다.
...
**Kind (Kubernetes IN Docker)**는 Docker 컨테이너를 노드로 사용하여 로컬에서 쿠버네티스 클러스터를 실행하는 도구이다.
...
- 몇 분 내에 로컬 클러스터 구성
- Control Plane + 여러 Worker 노드 구성 가능
- 다양한 K8s 버전 테스트 가능
...
kubectl은 쿠버네티스 클러스터를 관리하는 CLI 도구이다.
...
> **중요**: kubectl은 kube-apiserver의 **한 버전 차이(older or newer)**까지 지원된다.
>
...
**OCI**는 컨테이너 포맷과 런타임의 개방형 업계 표준을 만드는 거버넌스 기구이다.
...
컨테이너 이미지는 **레이어(Layer)**라는 읽기 전용 파일 시스템들이 쌓여있는 구조이다.
...
- 한 번 생성되면 변경 불가
- 동일한 베이스 레이어를 여러 이미지가 공유
- 변경된 레이어만 다운로드/업로드
- 컨테이너 실행 시 쓰기 가능 레이어 추가

### 3.1 도커를 사용한 컨테이너 빌드

#### 3.1.1 Dockerfile 이해하기

Dockerfile은 "사용자가 이미지를 조립하기 위해 명령행에서 호출하는 모든 명령을 담은 텍스트 문서"이다.
...
빌드와 런타임 환경을 분리하여 최종 이미지 크기를 줄이다.
...
**Jib (Java Image Builder)**는 Google이 개발한 Java 애플리케이션을 위한 컨테이너 이미지 빌더이다.

**Jib의 특징:**

- Docker를 설치하지 않아도 이미지 빌드 가능
- 변경된 레이어만 재빌드
- 동일한 입력에 대해 항상 같은 이미지 생성
- 빌드 도구에 직접 통합
...
```
Jib은 자동으로 애플리케이션을 3개의 레이어로 분리한다:
- 외부 라이브러리 (거의 변경 안됨)
- 설정 파일, static 파일
- 애플리케이션 클래스 (자주 변경됨)

이를 통해 코드 변경 시 애플리케이션 레이어만 재빌드하므로 빌드 시간이 크게 단축된다.

### 3.3 Buildah를 사용한 Daemonless 빌드

#### 3.3.1 Buildah 소개

**Buildah**는 Red Hat이 주도하는 오픈소스 프로젝트로, Docker 데몬 없이 OCI 컨테이너 이미지를 빌드하는 도구이다.
...
Buildah는 Podman과 함께 사용할 때 강력한다.
...
**Cloud Native Buildpacks (CNB)**는 CNCF 프로젝트로, 소스 코드에서 자동으로 컨테이너 이미지를 생성하는 도구이다.
...
- 개발자가 컨테이너화 신경 쓸 필요 없음
- 각 언어별 베스트 프랙티스 적용
- 베이스 이미지 업데이트 자동 적용
- 팀 전체가 동일한 빌드 프로세스 사용
...
**Shipwright**는 쿠버네티스에서 컨테이너 이미지를 빌드하는 프레임워크이다.
...
**Kaniko**는 쿠버네티스 환경에서 Docker 데몬 없이 Dockerfile을 빌드하는 도구이다.
...
**Kustomize**는 쿠버네티스 구성 파일을 **템플릿 없이** 커스터마이징하는 도구이다.
...
**Dragonfly**는 CNCF 프로젝트로, P2P (Peer-to-Peer) 기술을 사용하여 대규모 파일 및 컨테이너 이미지를 효율적으로 배포하는 시스템이다.
...
- GitOps 개념과 원칙 (단일 소스, 선언적, 자동 반영, 지속적 조정)
- CI/CD 파이프라인 (CI와 CD의 역할 분리, GitOps 워크플로)
- Docker Hub/Quay.io, Git, Kind 클러스터 환경 구성
- 컨테이너 이미지 빌드 (Docker, Jib, Buildah, Buildpacks, Shipwright/Kaniko)
- Kustomize를 활용한 Base/Overlay 패턴 및 환경별 설정 관리
