...
**ArgoCD ApplicationSet 마스터**
- ✅ List, Git, Matrix Generator 활용
- ✅ 멀티 환경/클러스터 자동 배포
- ✅ 모노레포 자동 앱 생성

**Sync Waves와 Hooks**
- ✅ 리소스 배포 순서 제어
- ✅ PreSync/PostSync Hook 활용
- ✅ DB 마이그레이션 자동화
...
#### 시나리오 1: 대규모 마이크로서비스 관리

**과제**: 50개 이상의 마이크로서비스를 3개 환경(dev, staging, prod)에 배포

**솔루션**:
1. **ApplicationSet Matrix Generator** 사용
   - 50개 앱 × 3개 환경 = 150개 Application 자동 생성
2. **AppProject**로 팀별 분리
   - Frontend팀, Backend팀, Data팀 각각 독립적인 프로젝트
3. **Sync Waves**로 의존성 관리
   - Database (Wave 1) → Backend (Wave 2) → Frontend (Wave 3)
4. **Notifications**로 배포 가시성 확보
   - 각 팀별 Slack 채널에 배포 알림
...
**솔루션**:
1. **Blue-Green 전략** 적용
   - 새 버전을 Preview 환경에 배포
   - 테스트 완료 후 수동 승인
   - Active 트래픽 전환
2. **PreSync Hook**으로 DB 백업
3. **PostSync Hook**으로 헬스체크
4. **RBAC**로 프로덕션 배포 권한 제한
   - 개발자: Sync만 가능
   - 운영팀: 프로모션 권한
5. **Sealed Secrets**로 시크릿 관리

#### 시나리오 3: AI/ML 모델 배포

**과제**: A/B 테스트를 통한 점진적 모델 배포

**솔루션**:
1. **Canary 전략** 활용
   - 신모델 10% → 30% → 50% → 100%
2. **AnalysisTemplate**으로 정확도 측정
   - Prometheus에서 모델 정확도 메트릭 수집
   - 정확도 < 90% 시 자동 롤백
3. **Git Generator**로 모델 버전 관리
   - models/v1/, models/v2/ 디렉토리별 자동 배포
...
# ApplicationSet 조회
kubectl get applicationset -n argocd
kubectl get appset -n argocd  # 단축형

# ApplicationSet 상세 정보
kubectl describe applicationset -n argocd <name>

# ApplicationSet으로 생성된 Application 조회
...
# Rollout 제어
kubectl argo rollouts promote <name>        # 다음 단계로 진행
kubectl argo rollouts abort <name>          # 롤아웃 중단
kubectl argo rollouts undo <name>           # 이전 버전으로 롤백
kubectl argo rollouts restart <name>        # 재시작
kubectl argo rollouts retry rollout <name>  # 실패한 롤아웃 재시도
...
kubectl get appproj -n argocd  # 단축형
