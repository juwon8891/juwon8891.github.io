...
| 용어 | 설명 |
|------|------|
| **HCA** | Host Channel Adapter - 서버용 InfiniBand NIC |
| **NVLink** | GPU 간 전용 고속 인터커넥트 (900 GB/s) |
| **NVSwitch** | NVLink 스위칭 칩 (All-to-all 연결) |
| **SXM** | Server eXtended Module - GPU 고성능 폼팩터 |
| **RDMA** | Remote Direct Memory Access - CPU 우회 직접 메모리 접근 |
...
- **CPU ↔ GPU**: PCIe, NVMe SSD, GPUDirect Storage
- **서버 내 GPU ↔ GPU**: NVLink, NVSwitch (SXM 폼팩터)
