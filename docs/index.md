# 학습 정리

Kubernetes, EKS, CUDA, CI/CD 등 다양한 기술 스택에 대한 학습 내용을 정리한 문서입니다.

## 주요 카테고리

### [ GPU](gpu/gpu-cluster-glossary/)
- [GPU 클러스터 & 분산 AI 용어 사전](gpu/gpu-cluster-glossary/) - RDMA, NCCL, vLLM/SGLang, NVIDIA vs AMD 비교
- [NVIDIA CUDA 컴파일 과정 및 GPU 아키텍처 심화](gpu/nvidia-cuda-architecture/)
- [InfiniBand, NVLink, NVSwitch 등 GPU 인터커넥트 기술](gpu/infiniband-nvlink-nvswitch/)

### [ EKS (Amazon Elastic Kubernetes Service)](eks/week1-intro-deployment/)
- [Week 1 - EKS 소개 및 배포](eks/week1-intro-deployment/)
- [Week 2 - Networking](eks/week2-networking/)
- [Week 3 - Scaling](eks/week3-scaling/)
- [Week 4 - AuthN & AuthZ](eks/week4-authn-authz/)
- [Week 5 - Pod Readiness Gates](eks/week5-pod-readiness-gates/)
- [Week 6 - CI/CD](eks/week6-cicd/)
- [Week 7 - Cluster Upgrade](eks/week7-cluster-upgrade/)

### [ K8s Deploy](k8s-deploy/week1-kubernetes-hard-way/)
- [Week 1 - Kubernetes Hard Way](k8s-deploy/week1-kubernetes-hard-way/)
- [Week 2 - Ansible Basics](k8s-deploy/week2-ansible-basics/)
- [Week 3 - Kubeadm Upgrade](k8s-deploy/week3-kubeadm-upgrade/)
- [Week 4 - Kubespray Deployment](k8s-deploy/week4-kubespray-deployment/)
- [Week 5 - Kubespray HA & Upgrade](k8s-deploy/week5-kubespray-ha-upgrade/)
- [Week 6 - Kubespray Offline](k8s-deploy/week6-kubespray-offline/)
- [Week 7 - RKE2 & Cluster API](k8s-deploy/week7-rke2-cluster-api/)

### [ CI/CD](cicd/week1-image-build/)
- [Week 1 - Image Build](cicd/week1-image-build/)
- [Week 2 - Helm & Tekton](cicd/week2-helm-tekton/)
- [Week 3 - Jenkins & ArgoCD](cicd/week3-jenkins-argocd/)
- [Week 4 - ArgoCD Advanced](cicd/week4-argocd-advanced/)
- [Week 5 - ArgoCD Security & Keycloak](cicd/week5-argocd-security-keycloak/)
- [Week 6 - ArgoCD Final](cicd/week6-argocd-final/)
- [Week 7 - Vault Secrets](cicd/week7-vault-secrets/)
- [Week 8 - Vault Production](cicd/week8-vault-production/)
