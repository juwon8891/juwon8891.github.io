...
DP는 소프트웨어의 수평 확장과 동일한 개념이다. 모델을 여러 벌 복제해 각 복제본이 트래픽의 일부만 처리하게 함으로써 처리량과 고가용성을 확보한다. 한 인스턴스가 다운되면 라우터가 나머지 인스턴스로 요청을 보낸다.
...
```mermaid
graph TB
    subgraph TP["TP=2 (너비 분할)"]
        L1A["Layer 1 절반"] --- L1B["Layer 1 절반"]
        L2A["Layer 2 절반"] --- L2B["Layer 2 절반"]
    end
    subgraph PP["PP=2 (깊이 분할)"]
        G1["GPU 1: Layer 1, 2"] --> G2["GPU 2: Layer 3, 4"]
    end
```
